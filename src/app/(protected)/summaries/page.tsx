...
export default async function SummariesPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-[28px] font-semibold text-[#1D1D1F] tracking-[-0.02em]">Summaries</h1>
        <p className="text-[16px] text-[#86868B] mt-1">AI-generated insights from your standups</p>
      </div>
      ...
      {!hasAny ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-[18px] font-semibold text-[#1D1D1F] mb-1">No summaries yet</p>
            <p className="text-[16px] text-[#86868B] mb-1">Weekly and monthly summaries are generated automatically after you complete standups.</p>
            <p className="text-[14px] text-[#9CA3AF] mb-5">Complete a full week of standups to get your first summary.</p>
            <a
              href="/standup"
        ...
        <>
          ...
          {monthly.length > 0 && (
            <section>
              <h2 className="text-[13px] font-semibold text-[#86868B] uppercase tracking-[1px] mb-4">Monthly</h2>
              <div className="space-y-3">
                {monthly.map((s) => (
                  <Card key={s.id} className="hover:border-accent/30 transition-colors">
                    <CardContent>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-0 mb-3">
                        ...
                        <div className="flex items-center gap-3 text-[14px] text-[#9CA3AF] pl-[18px] sm:pl-0">
                          <span>{s.standup_count} standups</span>
                          {s.streak_best != null && (
                            <span className="text-accent font-medium">{s.streak_best}-day best streak</span>
                          )}
                        </div>
                      </div>
                      <p className="text-[16px] text-[#4B5563] leading-relaxed">{s.summary}</p>
                      {s.highlights && (
                        <div className="mt-3 pt-3 border-t border-[#E5E5E5]">
                          <p className="text-[14px] font-medium text-[#86868B] uppercase tracking-[1px] mb-1.5">Highlights</p>
                          <p className="text-[16px] text-[#4B5563] leading-relaxed">{s.highlights}</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
          ...
          {weekly.length > 0 && (
            <section>
              <h2 className="text-[13px] font-semibold text-[#86868B] uppercase tracking-[1px] mb-4">Weekly</h2>
                ...
                {weekly.map((s) => (
                  <div key={s.id} className="px-4 sm:px-6 py-4 hover:bg-[#F5F5F7] transition-colors">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-[14px] text-[#9CA3AF]">{s.standup_count} standups</span>
                    </div>
                    <p className="text-[16px] text-[#4B5563] leading-relaxed">{s.summary}</p>
                  </div>
                ))}
